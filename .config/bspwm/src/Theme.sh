...
apply_zero_border() {
	PRESENTATION_MONITOR=$(cat "${HOME}/.config/bspwm/src/config/.no_polybar_mon")
	if [[ -n $PRESENTATION_MONITOR ]]; then
		bspc config -m $PRESENTATION_MONITOR top_padding 0
		bspc config -m $PRESENTATION_MONITOR bottom_padding 0
		bspc config -m $PRESENTATION_MONITOR left_padding 0
		bspc config -m $PRESENTATION_MONITOR right_padding 0
	fi
}
